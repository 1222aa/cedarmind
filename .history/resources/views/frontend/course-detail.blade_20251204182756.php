  ...
  <script>
    ...
    async function loadCourseDetails() {
      ...
      try {
        ...
        // fetch user's progress for this course first so UI can reflect completed items
        await fetchProgress();
    ...
    // Fetch per-course progress for current user
    async function fetchProgress() {
      ...
      try {
        ...
        completedIds = p.completed_ids || [];
        // update progress UI
        document.getElementById('lessonsCompleted').textContent = p.completed_count || 0;
        document.getElementById('averageScore').textContent = (p.percent !== undefined) ? `${p.percent}%` : '--';
        // also show total progress in the progress tab header or elsewhere as needed
        return p;
      } catch (e) {
    ...
    async function handleContentClick(e) {
      ...
      switch(content.type) {
        case 'video':
          modalContent.innerHTML = `
            ...
            <div class="mt-6">
              <button id="markCompleteBtn" class="bg-[var(--cedar)] text-white px-4 py-2 rounded">${completedIds.includes(content.id) ? 'Completed' : 'Mark as complete'}</button>
            </div>
          `;
          break;
        ...
        case 'written':
           modalContent.innerHTML = `
            <div class="prose max-w-none">
              ...
              `;
          // add mark complete button for written content
          setTimeout(() => {
            const btnWrap = document.createElement('div');
            btnWrap.className = 'mt-6';
            btnWrap.innerHTML = `<button id="markCompleteBtn" class="bg-[var(--cedar)] text-white px-4 py-2 rounded">${completedIds.includes(content.id) ? 'Completed' : 'Mark as complete'}</button>`;
            document.getElementById('modalContent').appendChild(btnWrap);
          }, 0);
            break;
        case 'embedquiz':
            modalContent.innerHTML = `
              <div class="prose max-w-none">
                <a data-quiz="QOO49DXFG" data-type="4" href="https://take.quiz-maker.com/QOO49DXFG">
                  Loading...
                </a>
              </div>
            `;
      ...
      // attach mark complete handler if present
      setTimeout(() => {
        const markBtn = document.getElementById('markCompleteBtn');
        if (markBtn) {
          markBtn.addEventListener('click', async () => {
            await markComplete(content.id, true);
    ...
    async function markComplete(contentId, completed = true) {
      const token = localStorage.getItem('cedarmind_token');
      try {
        const resp = await fetch('/api/progress/complete', {
          method: 'POST',
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
          ...
          body: JSON.stringify({ lesson_content_id: contentId, completed: completed })
        });
        return resp.ok;
      } catch (e) {
