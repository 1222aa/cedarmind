  ...
  <script>
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', async () => {
        const token = localStorage.getItem('cedarmind_token');
        console.log('Download clicked, courseId:', courseId);
        try {
          ...
          if (data.certificates && data.certificates.length > 0) {
            // Find certificate for current course by matching course title
            let cert = data.certificates.find(c => c.course_title && c.course_title.trim() === (courseData.title ? courseData.title.trim() : ''));
            
            // If no match found, use the first certificate
            if (!cert) {
              console.warn('No exact course match found, using first certificate');
    ...
    if (viewBtn) {
      viewBtn.addEventListener('click', async () => {
        const token = localStorage.getItem('cedarmind_token');
        console.log('View clicked, token:', token ? 'exists' : 'missing');
        try {
          const response = await fetch('/api/certificates', {
            headers: { 'Authorization': `Bearer ${token}` }
          });
      ...
      console.warn('viewCertificateBtn not found in DOM');
    }
