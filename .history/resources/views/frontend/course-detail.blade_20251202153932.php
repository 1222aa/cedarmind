      ...
      switch(content.type) {
        ...
        case 'written':
           modalContent.innerHTML = `
            ...
            break;
        case 'embedquiz':
  // insert the anchor element and button
          modalContent.innerHTML = `
            <div class="prose max-w-none">
              <a data-quiz="QOO49DXFG" data-type="4" href="https://take.quiz-maker.com/QOO49DXFG">
                Loading...
              </a>
              <button id="resetQuizBtn" class="mt-4 px-4 py-2 bg-[var(--cedar)] text-white font-semibold rounded hover:bg-[var(--light-cedar)]">
                Reset Quiz
              </button>
            </div>
          `;
          ...
          // reset quiz when button clicked
          const resetBtn = document.getElementById('resetQuizBtn');
          resetBtn.addEventListener('click', () => {
            // find the quiz anchor
            const quizAnchor = modalContent.querySelector('a[data-quiz]');
            if (quizAnchor && typeof qp !== 'undefined') {
              qp.startQuiz(quizAnchor); // restart the quiz
            }
          });
